...
## 句式格式（必须遵守）
- 每个任务单独成句成行，句式统一为「任务名：进度情况，子任务完成情况。」；
  句子以句号结尾，不得改写为「**分类名**：……」这类逐行分类前缀句式，
  也不得新增或保留「【分类】」标题、按分类归并任务。
- 保留原文已有的分节小标题（如「一、本周完成任务」「重点任务内容：」）及其顺序。
- 子步骤完成信息必须原样保留（如「进行中，4/7 子步骤已完成（……）」
  「子步骤「……」「……」均已完成」「完成子步骤 2/3 项」），不得删除或含糊化。
- 量化产出与数据必须原样保留。
...
def load_config(env_path: str | None = None) -> dict[str, str]:
    ...
    return {
        ...
        "model": env_vars.get("AI_MODEL", "deepseek-v4-flash"),
    }
...
def build_polish_prompt(summary_text: str) -> str:
    """Wrap *summary_text* in the 正式工作报告文风 prompt template."""
