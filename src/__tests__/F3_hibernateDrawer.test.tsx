...
describe('F3 — 休眠抽屉 (App.tsx → HibernateDrawer)', () => {
  ...
  describe('休眠任务列表渲染', () => {
    it('渲染休眠任务标题', () => {
      ...
      render(<HibernateDrawer tasks={tasks} onClose={vi.fn()} />);
    ...
    it('暂无休眠任务时显示空状态', () => {
      render(<HibernateDrawer tasks={[]} onClose={vi.fn()} />);
    ...
    it('关闭按钮可点击', () => {
      const onClose = vi.fn();
      const tasks = [makeHibernatingTask()];
      render(<HibernateDrawer tasks={tasks} onClose={onClose} />);
