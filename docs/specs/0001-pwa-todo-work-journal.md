...
4. 年底从全年记录生成年度总结，按日常工作、内部招聘、奖惩管理、劳动关系等维度归纳，支撑评优和晋升述职
5. 在办公电脑（只能上受限环境（不便安装额外软件））和个人安卓手机之间同步数据
...
1. 你在办公电脑或手机上通过 PWA 管理任务（增删改查、标记完成）
...
5. 办公电脑 PWA 刷新即可看到润色结果，在此基础上手动编辑定稿

## User Stories

### 每日任务管理

1. 作为用户，我想在桌面独立窗口看到我的任务清单，顶部有紧急区，下方按优先级排列，一目了然
2. 作为用户，我想添加新任务时填写标题、分类、优先级、截止日期、量化产出指标、关联项目、备注
3. 作为用户，我想把任务标记为紧急，紧急任务自动置顶到紧急区（最多 5 条），避免视觉过载
4. 作为用户，我想勾选完成任务，已完成的任务显示为划线/淡化，仍可取消勾选恢复
5. 作为用户，我想取消的任务能归档而非删除，保留完整记录
6. 作为用户，我想随时拖拽或调整任务顺序，按当日需要重新排列轻重缓急
7. 作为用户，我想看到每个长期父任务的子任务数量（如"8/12"进度条），知道推进了多少
8. 作为用户，我想给"交办事项"类任务打特殊标记，记录交办来源、交办时间、完成时限

### 长期项目追踪

9. 作为用户，我想创建"项目"作为父任务容器，下面建子任务追踪推进
10. 作为用户，我想看到父任务根据子任务完成数量自动显示进度条，但最终完成需要手动确认
11. 作为用户，我想在周报中看到长期项目的进度变化（从 X% → Y%），让推进感可见

### 周/月/年度小结

12. 作为用户，我想每周五系统自动提醒生成周小结，无需记忆
13. 作为用户，我想周小结自动拉入本周完成的任务（按分类排列），拉入下周待办，汇总阻塞事项
14. 作为用户，我想在周小结"下周计划"中手动补充新任务，新任务自动回填到任务清单
15. 作为用户，我想月小结自动汇总当月量化产出（各类别统计表）、长期项目进度回顾，并可手写反思和下月重点
16. 作为用户，我想年度报告按日常工作、项目推进、奖惩管理、绩效管理、劳动关系、交办事项等维度自动归纳，列出每个维度的关键业绩和工作量
17. 作为用户，我想手动对年度报告的每个维度添加"关键业绩提炼"文字，体现深度

### AI 润色

18. 作为用户，我想在个人电脑上运行一个 Python 脚本，自动读取 `data.json` 中待润色的小结，调用 AI API 润色语言后写回
19. 作为用户，我想 AI 润色保持正式工作报告文风——正式、简洁、结构化；同时你可以控制是否启用 AI 润色
20. 作为用户，我想 AI 润色后的结果仍可手动编辑，不完全依赖 AI 输出
21. 作为用户，我想在年终时把年度小结草稿 + 团队年度工作计划一起交给 AI 做"计划对齐"分析，体现工作契合组织发展

### 提醒机制

22. 作为用户，我想重要任务有截止倒计时提醒（如"距 X 项目截止还有 15 天"）
23. 作为用户，我想跨周跨月的重要任务持续显示提醒，不被遗忘
24. 作为用户，我想标记为"跨年"的任务可以设置休眠，在临近截止前 1-2 个月才出现，平时不占视觉空间

### 跨设备同步

25. 作为用户，我想在办公电脑上改动任务后，手机上打开 PWA 就能看到最新状态
26. 作为用户，我想 WPS 作为同步中转，不需要任何额外服务器或安装软件
27. 作为用户，我想数据 JSON 文件在 WPS 云文档文件夹里，可以作为备份随时拷贝

### 数据导出

28. 作为用户，我想周报内容一键复制为纯文本，方便粘贴到 OA 系统或邮件
29. 作为用户，我想数据可以导出为 Excel，方便离线备份或发给别人
30. 作为用户，我想年度报告有附表（月度趋势折线图、量化产出汇总表），供翻看记录时参考

### 配置

31. 作为用户，我想要一个设置页面，可以管理分类列表（日常工作、项目推进、协作沟通、会议培训、临时交办、其他）
32. 作为用户，我想要设置页面可以配置周结日（默认周五）、月结日（默认每月28号）
33. 作为用户，我想要事后补录功能——周/月归档后仍可编辑修改数据

### 数据安全

34. 作为用户，我想数据存在自己的 WPS 云文档里，不经过任何第三方服务器（AI API 调用除外，且仅在我个人电脑上触发）
35. 作为用户，我想办公电脑上只有 WPS Office（办公软件）和浏览器打开 PWA，无任何额外进程，不会被受管环境标记
...
- 可配置：API 端点、模型名、prompt 模板（预设"正式工作报告文风"模板）
...
- 年度上级计划"计划对齐"功能内嵌（事后交给 AI 独立处理，不在 PWA 中）
- 通知推送（依赖系统级通知不可靠）
- iOS 支持（用户使用安卓手机）
...
- AI 润色默认"正式工作报告文风"——正式、简洁、结构化、避免口语化
